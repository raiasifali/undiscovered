...
const ContactInformationForm = () => {
  ...
  const handleInputChangeSub = (e, v) => {
    ...
    let updated = (state.coach.philosophyPlayingStyle[v] = value);
  ...
  const onAddRow = () => {
    ...
    let updated = oldData.philosophyPlayingStyle.push('');
  ...
  const { philosophyPlayingStyle } = state.coach;

  return (
    <div className="contactInformationForm">
      <h2 style={{ fontSize: '22px' }}>Philosophy & Playing Style</h2>
      <div className="form-data">
        <div className="coachFields">
          <label style={{ fontSize: '16px' }} className="pb-5">
            Bullets
          </label>
          {philosophyPlayingStyle.map((team, i) => (
            <div className="flex gap-3 mt-5">
              ...
              {philosophyPlayingStyle.length - 1 === i ? (
                <button
                  onClick={onAddRow}
